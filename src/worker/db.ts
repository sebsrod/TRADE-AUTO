...
export async function upsertSuggestion(env: Env, s: SuggestionInput): Promise<Suggestion> {
  ...
  if (existing) {
    ...
    // Collapse any OTHER pending duplicates for this symbol into the row we just
    // updated, so the hub shows exactly one entry per asset even if older
    // duplicates accumulated before dedup was in place.
    await env.DB.prepare(
      `UPDATE suggestions SET status = 'expired'
       WHERE user_id = ? AND symbol = ? AND status = 'pending' AND id != ?`,
    )
      .bind(s.user_id, s.symbol, existing.id)
      .run();
    return updated;
  }
