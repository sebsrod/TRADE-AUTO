...
// One position row, split out so each can flash its live price / P&L independently
// as the ~5s quote poll updates the numbers.
function PositionRow({
  ...
  return (
    <tr className={`clickable ${selected ? "selected" : ""}`} onClick={() => onSelect(p)} title="View chart & reason">
      ...
      <td className={`num ${priceFlash}`}>
        {p.current_price == null ? "—" : fmtNum(p.current_price, 2)}
      </td>
      <td className="num sub">
        {fmtNum(p.stop_loss, 2)} / {fmtNum(p.take_profit, 2)}
      </td>
      <td className={`num ${pnlClass(p.unrealized_pnl)} ${pnlFlash}`}>
...
// Drop-in replacement for PortfolioPanel with live-flashing rows.
